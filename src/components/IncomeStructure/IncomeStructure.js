...
const IncomeStructure = ({chartData, checked, hideFilter, setChecked}) => {
  ...
  useEffect(() => {
    if (chartData?.length) {
      ...
      const groupedData = chartData.reduce((result, item, index) => {
        if (item?.accountID?.accountName) {
          const existingItem = result.find(
            i => i?.label === item?.accountID?.accountName,
          );

          if (existingItem !== undefined) {
            result = result.map(obj =>
              obj?.label === item?.accountID?.accountName
            ...
            result.push({
              ...
              key: `pie-${index}`,
              label: item?.accountID?.accountName,
            });
          }
        }
        return result;
  ...
  return (
    <View style={styles.container}>
      ...
      {mydata?.length ? (
        ...
        <CustomText textStyle={styles.nodata}>No Income Available</CustomText>
      )}
    </View>
  );
};
